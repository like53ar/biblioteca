...
objFile.WriteLine ".progress-fill { background: linear-gradient(90deg, #667eea 0%, #764ba2 100%); height: 100%; width: 0%; animation: progress 2s linear forwards; }"
...
objFile.WriteLine "<div class='status' id='status'>Iniciando...</div>"
objFile.WriteLine "<script>"
objFile.WriteLine "window.resizeTo(450, 250);"
objFile.WriteLine "window.moveTo((screen.width - 450) / 2, (screen.height - 250) / 2);"
objFile.WriteLine "setTimeout(function() { window.close(); }, 2500);"
objFile.WriteLine "</script>"
...
' Iniciar el servidor (oculto)
WshShell.Run "cmd /c cd /d """ & scriptPath & "\server"" && npm start", 0, False

' Esperar 2 segundos para asegurar el inicio del proceso
WScript.Sleep 2000
